...
class Program
{
  ...
  public class Options
  {
    ...
    [Option("allow-doubles", Required = false, Default = false, HelpText = "Skip duplicate-element guard when applying <add> operations.")]
  ...
  )
  {
    ...
    foreach (var diffFile in Directory.EnumerateFiles(diffDir, "*.xml", SearchOption.AllDirectories))
    {
  ...
  )
  {
    ...
    try
    {
      ...
      {
        Logger.Error($"Root element of diff is not 'diff'. Found: '{diffRoot?.Name}'. Skipping.");
        return;
      }
      ...
      foreach (var node in diffRoot.Nodes())
      {
        ...
        switch (operation.Name.LocalName)
        {
          ...
          case "remove":
            ...
            break;
          default:
            Logger.Warn($"Unknown diff operation '{operation.Name.LocalName}'. Skipping.");
            break;
        }
      }
