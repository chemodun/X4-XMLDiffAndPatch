...
public static class XmlUtils
{
    ...
    public static void ConfigureLogging(string? logToFile, bool appendToLog)
    {
        ...
        var fileLevel = string.IsNullOrEmpty(logToFile) ? LogLevel.Info : ParseLogLevel(logToFile);
        var consoleLevel = fileLevel < LogLevel.Info ? fileLevel : LogLevel.Info;
    ...
    public static XmlReaderSettings? CreateXmlReaderSettings(string xsdPath)
    {
        ...
        {
            Logger.Warn(
                $"Failed to load XSD '{xsdPath}': {ex.Message}. Validation will be skipped."
            );
            return null;
        }
    }
