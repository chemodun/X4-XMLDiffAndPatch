...
public class DiffEngine
{
    ...
    )
    {
        ...
        {
            var (_, rootSavedOp) = CompareAttributes(
                original.Root!,
                modified.Root!,
                checkOnly: false
            );
        ...
        while (i < originalChildren.Count && j < modifiedChildren.Count)
        {
            ...
            {
                var (attrMatched, savedOp) = CompareAttributes(
                    originalChild,
                    modifiedChild,
                    checkOnly
                );
                ...
                if (matchedEnough && savedOp != null)
                {
                    ...
                    bool childrenMatch = !CompareElements(
                        original,
                        modified,
                        diffRoot,
                        originalChild,
                    ...
                    bool siblingsAlign =
                        ...
                        || (
                            ...
                            && CompareAttributes(
                                originalChildren[i + 1],
                                modifiedChildren[j + 1],
                                checkOnly: true
                            ).matchedEnough
                        );
            ...
            else
            {
                ...
                for (int k = j + 1; k < modifiedChildren.Count; k++)
                {
                    if (ExactlyMatches(modifiedChildren[k], originalChild))
                    {
                        ...
                        Logger.Info(
                            $"[Operation add] {k - j} element(s) before {XmlUtils.GetElementInfo(originalChild)}"
                        );
                        j = k;
                        foundMatch = true;
                        break;
                ...
                if (!foundMatch)
                {
                    ...
                    bool nextOriginalFoundLaterInModified =
                        ...
                        && modifiedChildren
                            .Skip(j + 1)
                            .Any(mc => ExactlyMatches(mc, originalChildren[i + 1]));

                    bool nextOriginalIsCurrentModified = originalChildren
                        .Skip(i + 1)
                    ...
                    bool shouldReplace =
                        ...
                        && (
                            (
                                ...
                                && originalChild
                                    .Attributes()
                                    .Any(a => modifiedChild.Attribute(a.Name)?.Value == a.Value)
                            )
                            || i + 1 == originalChildren.Count
    ...
    )
    {
        ...
        var originalAttrs = originalElement
            .Attributes()
            .ToDictionary(a => a.Name.LocalName, a => a.Value);
        var modifiedAttrs = modifiedElement
            .Attributes()
            .ToDictionary(a => a.Name.LocalName, a => a.Value);
        ...
        foreach (var kvp in modifiedAttrs)
        {
            ...
            if (!originalAttrs.TryGetValue(kvp.Key, out var origValue))
            {
                ...
                if (!checkOnly)
                {
                    ...
                    savedOp = new XElement(
                        "add",
                        new XAttribute("sel", xpath),
            ...
            else if (origValue != kvp.Value)
            {
                ...
                if (!checkOnly)
                {
                    ...
                    savedOp = new XElement(
                        "replace",
                        new XAttribute("sel", $"{xpath}/@{kvp.Key}"),
                        kvp.Value
                    );
                }
            }
    ...
    )
    {
        ...
        {
            bool usePosBeforeComment = XmlUtils.IsElementPrecededByPosBeforeComment(
                modifiedChildren[j]
            );
